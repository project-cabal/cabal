...
bool Surface::loadIFF(Common::SeekableReadStream &stream) {
	...
	// FIXME: This assumes width == pitch

	resize(decoder.getSurface()->getWidth(), decoder.getSurface()->getHeight());
	memcpy(_vidMem, decoder.getSurface()->getPixels(), decoder.getSurface()->getWidth() * decoder.getSurface()->getHeight());
