	...
	if (_decoder.isVideoLoaded()) {
		if (_decoder.endOfVideo()) {
			...
			if (s) {
				...
				assert(s->getFormat().bytesPerPixel == 4);

#ifdef THEORA_INDIRECT_RENDERING
				const byte *frameData = (const byte *)s->getPixels();
				_outputBitmap->setContent(frameData, s->getPitch() * s->getHeight(), 0, s->getPitch());
#else
				g_system->copyRectToScreen(s->getPixels(), s->getPitch(), _outX, _outY, MIN(s->getWidth(), _backSurface->getWidth()), MIN(s->getHeight(), _backSurface->getHeight()));
